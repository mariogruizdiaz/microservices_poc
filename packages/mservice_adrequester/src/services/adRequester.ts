...
export class AdRequester {
    ...
    public static INSTANCE_ID = `adreq_${
    ...
    public static async serviceImplementation(
        ...
        if (message.reply) {
            ...
            for (let i = 0; i < total; i++) {
                ads.push(
                    `Ad #${++adNumber} from ${
                        AdRequester.INSTANCE_ID
                    }.instance for the url ${message.data}`
                );
            }

            MessagingService.response(
                AdRequester.serviceName,
                message.reply,
                new AdRequesterResponse(200, 'Everything OK', { ads })
            );
            return Promise.resolve();
        } else {
            return Promise.reject(
                `Error sending request Message. Use "request" instead of "publish" for expecting a response of it. Message: ${JSON.stringify(
                    message
                )}`
            );
        }
